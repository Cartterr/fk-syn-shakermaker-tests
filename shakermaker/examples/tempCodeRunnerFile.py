...
# Extract data using ZENTPlot function
t, a_manual, v_manual, d_manual = ZENTPlot(s1, show=False)

# Compute the integral of the acceleration data to get velocity
v_integral = cumulative_trapezoid(a_manual, t, initial=0)

# Compute the integral of the velocity data to get displacement
d_integral = cumulative_trapezoid(v_integral, t, initial=0)

# Compare the manually calculated integrals with shakermaker's results
print("Manually Calculated vs Shakermaker's Results")
print("--------------------------------------------")
print(f"Velocity: Manual Integral: {v_integral[-1]:.4f}, Shakermaker: {v_manual[-1]:.4f}")
print(f"Displacement: Manual Integral: {d_integral[-1]:.4f}, Shakermaker: {d_manual[-1]:.4f}")
